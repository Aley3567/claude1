...
- 仅读取、解释或盘点时不创建提交。用户要求修改文件时，只要当前改动形成了可独立验证的完整行为，默认主动创建本地 commit 并汇报，不再要求用户重复下达 Git 指令。
- 本地行为级 commit 不授权 push、merge、rebase、amend、强制更新 ref、删除分支或清理 worktree；这些动作必须由用户明确要求。
...
开始编辑前向用户简短报告分支、upstream 差异、与本任务相关的既有修改，以及预期形成的 checkpoint。不要把完整 status 倾倒给用户；只报告影响本任务判断的部分。
