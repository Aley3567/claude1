...
/// 本地状态文件的可读范围。
///
/// Windows 上这一条只能如实说「查不了」：访问控制是 ACL，没有 unix 的权限位可读，
/// 桌面端也不解析 ACL（要么引一个新依赖，要么解析 `icacls` 的本地化输出，两条都不靠谱）。
/// 所以这里既不报 ok 也不报 fail——报 ok 就是凭空断言「只对你开放」，
/// 那正是 AGENTS.md 禁止的把没查过的事说成查过了。
///
/// 同时把用户真正能自己核对的路径列出来，让「查不了」是一条可行动的结论而不是一句推脱。
fn check_file_permissions(out: &mut Vec<DoctorCheck>) {
    ...
    for file in files {
        if file.is_file() {
            watched.push(error::tilde(&file));
        }
    }
    watched.sort();
    watched.dedup();
    let listed = if watched.is_empty() {
        "目前这个目录下还没有需要保护的配置文件。".to_string()
    } else {
        format!("需要留意的文件：{}。", watched.join("、"))
    };
    out.push(check(
        "file-permissions",
        "info",
        "本地状态文件的可读范围这台机器上查不了",
        Some(format!(
            "Windows 用 ACL 而不是 unix 权限位，桌面端不解析 ACL，所以不能断言 {} 下的东西只对你开放。默认情况下 %USERPROFILE% 里的文件只有当前用户、SYSTEM 与管理员能访问；这台机器多人共用、或者用户目录的权限被改过的话，请在「属性 → 安全」里或用 icacls 自己核对一遍。hub 配置里带着本机 token，值得核。{}",
            error::tilde(&dir),
            listed
        )),
        None,
...
pub fn fix_subagent_pins() -> Result<Vec<DoctorCheck>, String> {
    ...
    // Windows 上 python3 的名字与调用形式不止一种（`py -3` / `python`），
    // 所以解析交给 env::locate_python，它会把前置参数一起带回来。
    let python = env::locate_python().ok_or_else(|| {
        "没找到 python3（py -3、python、python3 都试过了），无法调用 claude1 doctor --fix 清理子代理固定值"
            .to_string()
    })?;
