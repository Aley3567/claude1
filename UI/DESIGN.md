...
输入框在顶部无边框，下方分组结果（`最近使用` / `导航` / `渠道` / `槽位` / `动作` 五组；
`最近使用` 记录最近 5 条已执行命令置顶），↑↓ 移动、Enter 执行、Esc 关闭；每组显示
条数有上限，超出的折叠成「还有 N 条未显示」，靠继续输入收窄。模糊匹配（子序列匹配
即可，不引 fuse.js）。命中的字符用 `--accent-text` 高亮。IME 组合中的按键不当作
命令（Enter 是选词不是执行）。
...
### 4.8 反馈与 toast（`store/toast.ts` + `shell/ToastLayer.tsx`）

轻量即时反馈：右下角浮层，`--z-toast`（在命令面板之上），success / error 两档
样式，约 3 秒自动消失，aria-live 播报同一文本。**toast 只是轻反馈，不承担留痕**：
结论性信息（半失败结果、需要用户回头看的详情）必须在视图里另留持久的一份——
toast 3 秒后无痕消失，留痕是页面内元素的职责（如 doctor 视图的 fixNote）。

### 4.9 长视图的分段与锚点

长列表视图（诊断）用**吸顶小节头 + 侧栏锚点导航**维持方位感：滚动时小节头 sticky
钉住（底必须不透明，`--bg-base`，防列表行透出），小节 `id` 供锚点跳转；吸顶偏移量
一律从 token 推导（控件高度 + 工具条 padding），不写死 px——控件尺寸因平台而异，
写死会在两条吸顶栏之间露缝。锚点跳转的落点用 `scroll-margin-top` 给吸顶层留位。
